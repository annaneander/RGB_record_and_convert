...
uint16_t rgbc[4] = {0}; /* value of RGBC */
...
volatile uint8_t* p_read_from_reg(uint8_t reg){
	...
	*data = i2c_recv();
	display_debug(&I2C1RCV);    // 0xAB
	display_debug_2(data);    // 0xA0003FAB och ibland 30
	/*
...
/* read 4 * 2 bytes from register (reg) in sensor
   RGBC */
...
bool send_data(uint8_t data){
	bool status = 0;
	volatile uint8_t *id_p;
	int *temp;
	uint8_t id;

	//snabbtest för kontakt:
	id_p = p_read_from_reg(DEVICE_ID);
	//display_debug_2(id_p); // värde: 0x30
	if (*id_p == 0xAB)
		display_string(0,"ok pekare AB"); //detta stämmer ibland
	display_update();


	id = read_from_reg(DEVICE_ID);
	display_debug_8(&id); // värde: 0xAB ibland
	if (id == 0xAB)
		display_string(0,"ok int AB"); //detta stämmer ibland
	display_update();
...
/* test med global array.
   read n bytes from register  (reg) in sensor */
void rgbc_from_reg(uint8_t reg){
  ...
  char * value;
   ...
   //läs CRGB: Alla värden på CRGB skickas efter varandra, som 2x2 byte (totalt 8 byte) om läsningen börjar med adressen för C_LOW.
   for(i = 0; i<4; i++) {
 *(rgbc+i) = i2c_recv() <<8;
    i2c_ack();
 *(rgbc+i) |= i2c_recv();
